...
WavePI<dim, Meas>::WavePI(std::shared_ptr<SettingsManager> cfg)
      : cfg(cfg) {
   ...
   norm_h2l2plusl2h1 = std::make_shared<norms::H2L2PlusL2H1<dim>>(cfg->norm_h2l2plusl2h1_alpha,
         cfg->norm_h2l2plusl2h1_beta, cfg->norm_h2l2plusl2h1_gamma);
...
void WavePI<dim, Meas>::initialize_problem() {
   ...
   case SettingsManager::ProblemType::rho:
      ...
      problem = std::make_shared<RhoProblem<dim, Meas>>(*wave_eq, pulses, measures, transform,
            param_background_discretized);
      break;
   default:
      AssertThrow(false, ExcInternalError())
...
void WavePI<dim, Meas>::log_error_initial(DiscretizedFunction<dim>& reconstruction_minus_initial,
      std::shared_ptr<Norm<DiscretizedFunction<dim>>> norm, DiscretizedFunction<dim>& exact_minus_initial) {
...
void WavePI<dim, Meas>::run() {
   ...
   case SettingsManager::NonlinearMethod::reginn:
      regularization = std::make_shared<REGINN<Param, Tuple<Meas>, Exact>>(problem, initial_guess_discretized,
            *cfg->prm);
      break;
      ...
      if (Utilities::MPI::n_mpi_processes(MPI_COMM_WORLD) > 1) {
         ...
         deallog << "  lin forward     : avg "
               << Util::format_duration(
                     stats->time_linearization_forward_communication / stats->calls_linearization_forward)
               << " per call, " << std::fixed << std::setprecision(2)
               << (stats->time_linearization_forward_communication / timer_inversion.wall_time() * 100)
               << "% of total time" << std::endl;
         deallog << "  lin adjoint     : avg "
               << Util::format_duration(
                     stats->time_linearization_adjoint_communication / stats->calls_linearization_adjoint)
               << " per call, " << std::fixed << std::setprecision(2)
