...
FibonacciMaxIterChoice::FibonacciMaxIterChoice(int initial_max_iter) : initial_max_iter(initial_max_iter) {}

FibonacciMaxIterChoice::FibonacciMaxIterChoice(ParameterHandler &prm) { get_parameters(prm); }
...
int FibonacciMaxIterChoice::calculate_max_iter() const {
  if (required_steps.size() == 0) return initial_max_iter;

  if (required_steps.size() == 1) return 2 * initial_max_iter;

  return required_steps[required_steps.size() - 1] + required_steps[required_steps.size() - 2];
}
