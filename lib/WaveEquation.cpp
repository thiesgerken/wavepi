...
WaveEquation<dim>::WaveEquation(DoFHandler<dim> *dof_hndl) :
		theta(0.5), time_end(1), time_step(1. / 64), backwards(false), dof_handler(
				dof_hndl), initial_values_u(&zero), initial_values_v(&zero), boundary_values_u(
				&zero), boundary_values_v(&zero), param_c(&one), param_nu(
				&zero), param_a(&one), param_q(&zero), right_hand_side(
				&zero_rhs) {
}
...
void WaveEquation<dim>::setup_step(double time) {
	...
	// tests show about 25% speedup on my Intel i5 4690
...
DiscretizedFunction<dim> WaveEquation<dim>::run() {
	...
	for (int i = 1;
			(!backwards && time < time_end) || (backwards && time > 0.0); i++) {
	...
	timer.stop();
	deallog << "solved pde in " << std::fixed << timer.wall_time()
			<< "s (setup " << setup_timer.wall_time() << "s)" << std::fixed
			<< std::endl;

	return u;
}

template<int dim> void WaveEquation<dim>::set_initial_values_u(
		Function<dim>* values_u) {
	initial_values_u = values_u;
}

template<int dim> void WaveEquation<dim>::set_initial_values_v(
		Function<dim>* values_v) {
...
template<int dim> void WaveEquation<dim>::set_boundary_values_u(
		Function<dim>* values_u) {
	initial_values_u = values_u;
}

template<int dim> void WaveEquation<dim>::set_boundary_values_v(
		Function<dim>* values_v) {
	initial_values_v = values_v;
}
...
template<int dim> void WaveEquation<dim>::set_right_hand_side(
