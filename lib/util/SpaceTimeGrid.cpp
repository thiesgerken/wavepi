...
SpaceTimeGrid<2>::SpaceTimeGrid(const std::vector<double> &times,
   ...
   for (size_t ix = 0; ix < spatial_points[0].size(); ix++)
      for (size_t iy = 0; iy < spatial_points[1].size(); iy++)
         points_per_time[ix * spatial_points[1].size() + iy] = Point<2>(spatial_points[0][ix],
               spatial_points[1][iy]);
   ...
   for (size_t ix = 0; ix < spatial_points[0].size(); ix++)
      for (size_t iy = 0; iy < spatial_points[1].size(); iy++)
         for (size_t iz = 0; iz < spatial_points[2].size(); iz++)
            points_per_time[ix * spatial_points[1].size() * spatial_points[2].size()
                  + iy * spatial_points[2].size() + iz] = Point<3>(spatial_points[0][ix],
                  spatial_points[1][iy], spatial_points[2][iz]);
