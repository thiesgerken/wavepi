...
WaveEquation<dim>::WaveEquation() :
		initial_values_u(&zero), initial_values_v(&zero), boundary_values_u(
				&zero), boundary_values_v(&zero), right_hand_side(&zero), param_c(
				&one), param_nu(&zero), param_a(&one), param_q(&zero), theta(
				0.5), time_end(1), time_step(1. / 64), fe(1), dof_handler(
...
void WaveEquation<dim>::run() {
	...
	// projecting might make more sense, but VectorTools::project leads to a mutex error (deadlock) on my laptop (Core i5 6267U)
