...
template<int dim>
AssemblyScratchData<dim>::AssemblyScratchData(const FiniteElement<dim> &fe, const Quadrature<dim> &quad)
...
template<int dim>
void local_assemble_cc(const Function<dim> * const f1, const Function<dim> * const f2,
      const typename DoFHandler<dim>::active_cell_iterator &cell, AssemblyScratchData<dim> &scratch_data,
   ...
   for (unsigned int q_point = 0; q_point < n_q_points; ++q_point) {
      ...
      if (f2 != nullptr) {
         const double val2 = f2->value(scratch_data.fe_values.quadrature_point(q_point));
   ...
   if (f1_d != nullptr && f2_d != nullptr)
      WorkStream::run(dof.begin_active(), dof.end(),
            std::bind(&local_assemble_cc<dim>,
                  std::ref(f1_d->get_function_coefficient(f1_d->get_time_index())),
      ...
      WorkStream::run(dof.begin_active(), dof.end(),
            std::bind(&local_assemble_dc<dim>,
                  std::ref(f1_d->get_function_coefficient(f1_d->get_time_index())), f2,
