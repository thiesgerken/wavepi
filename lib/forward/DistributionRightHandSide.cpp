...
template<int dim>
DistributionRightHandSide<dim>::AssemblyScratchData::AssemblyScratchData(const FiniteElement<dim> &fe, const Quadrature<dim> &quad)
...
void DistributionRightHandSide<dim>::create_right_hand_side(const DoFHandler<dim> &dof,
   ...
   if (f1_d != nullptr && f2_d != nullptr)
      ...
      WorkStream::run(dof.begin_active(), dof.end(),
            std::bind(&DistributionRightHandSide<dim>::local_assemble_cc, f1, f2, std::placeholders::_1, std::placeholders::_2,
                  std::placeholders::_3),
            std::bind(&DistributionRightHandSide<dim>::copy_local_to_global, std::ref(rhs), std::placeholders::_1),
            AssemblyScratchData(dof.get_fe(), quad), AssemblyCopyData());
}
