...
template <int dim>
std::shared_ptr<SparsityPattern> AdaptiveMesh<dim>::get_sparsity_pattern(size_t idx) {
  Assert(idx >= 0 && idx < this->length(), ExcIndexRange(idx, 0, this->length()));
...
template <int dim>
std::shared_ptr<DoFHandler<dim>> AdaptiveMesh<dim>::transfer(size_t source_time_index, size_t target_time_index,
...
void AdaptiveMesh<dim>::transfer(size_t target_idx, std::vector<Vector<double> *> &vectors) {
  ...
  if (target_idx != working_time_idx)
    deallog << working_time_idx << " → " << target_idx << ", taking " << vectors.size() << " vector(s) along"
            << std::endl;
...
size_t AdaptiveMesh<dim>::memory_consumption() const {
  size_t mem =
      ...
      MemoryConsumption::memory_consumption(initial_triangulation) + MemoryConsumption::memory_consumption(times) +
      MemoryConsumption::memory_consumption(sparsity_patterns) + MemoryConsumption::memory_consumption(mass_matrices) +
      MemoryConsumption::memory_consumption(vector_sizes) + MemoryConsumption::memory_consumption(working_time_idx);

  for (size_t i = 0; i < this->length(); i++) {
    if (mass_matrices[i]) mem += MemoryConsumption::memory_consumption(*mass_matrices[i]);

    if (sparsity_patterns[i]) mem += MemoryConsumption::memory_consumption(*sparsity_patterns[i]);
  }
