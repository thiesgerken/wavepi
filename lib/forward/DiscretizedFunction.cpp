...
template<int dim>
void DiscretizedFunction<dim>::push_back(DoFHandler<dim>* dof_handler,
		double time, const Vector<double>& function_coeff) {
...
template<int dim>
void DiscretizedFunction<dim>::push_back(DoFHandler<dim>* dof_handler,
		double time, Function<dim>& function) {
	Assert(!store_derivative, ExcInvalidState());
...
DiscretizedFunction<dim>::DiscretizedFunction(const DiscretizedFunction&& o) :
		store_derivative(o.store_derivative), cur_time_idx(o.cur_time_idx), times(
				std::move(o.times)), dof_handlers(std::move(o.dof_handlers)), function_coefficients(
				std::move(o.function_coefficients)), derivative_coefficients(
				std::move(o.derivative_coefficients)) {
}

template<int dim>
DiscretizedFunction<dim>::DiscretizedFunction(const DiscretizedFunction& o) :
		Function<dim>(), store_derivative(o.store_derivative), cur_time_idx(
				o.cur_time_idx), times(o.times), dof_handlers(o.dof_handlers), function_coefficients(
				o.function_coefficients), derivative_coefficients(
				o.derivative_coefficients) {
}

template<int dim>
DiscretizedFunction<dim>::DiscretizedFunction(Function<dim>& function,
...
template<int dim>
DiscretizedFunction<dim>::DiscretizedFunction(const std::vector<double>& times,
...
void DiscretizedFunction<dim>::reverse() {
	...
	std::reverse(derivative_coefficients.begin(),
			derivative_coefficients.end());
}
...
template<int dim>
DiscretizedFunction<dim> DiscretizedFunction<dim>::noise(
		const DiscretizedFunction<dim>& like, double norm) {
...
template<int dim>
void DiscretizedFunction<dim>::pointwise_multiplication(
		const DiscretizedFunction<dim> & V) {
...
template<int dim>
double DiscretizedFunction<dim>::l2_dot(
		const DiscretizedFunction<dim> & V) const {
	...
	for (size_t i = 0; i < times.size(); i++) {
		...
		double dot = function_coefficients[i] * V.function_coefficients[i];

		if (i > 0)
			result += dot / 2 * (std::abs(times[i] - times[i - 1]));

		if (i < times.size() - 1)
			result += dot / 2 * (std::abs(times[i + 1] - times[i]));
	}
...
template<int dim>
void DiscretizedFunction<dim>::write_pvd(std::string path,
...
void DiscretizedFunction<dim>::write_pvd(std::string path, std::string name,
	...
	for (size_t i = 0; i < times.size(); i++) {
		const std::string filename = path + "-" + Utilities::int_to_string(i, 4)
				+ ".vtu";
		times_and_names.push_back(
				std::pair<double, std::string>(times[i], filename));

		task_group += Threads::new_task(&DiscretizedFunction<dim>::write_vtk,
				*this, name, name_deriv, filename, i);
	}
...
template<int dim>
void DiscretizedFunction<dim>::write_vtk(const std::string name,
		const std::string name_deriv, const std::string filename,
		size_t i) const {
...
bool DiscretizedFunction<dim>::near_enough(double time, size_t idx) const {
	...
	if (times.size() == 1)
		...
		return std::abs(times[idx] - time)
				< 1e-3 * std::abs(times[idx] - times[idx - 1]);
	else
		return std::abs(times[idx] - time)
				< 1e-3 * std::abs(times[idx + 1] - times[idx]);
}
...
size_t DiscretizedFunction<dim>::find_time(double time) const {
	...
	if (!near_enough(time, idx)) {
		...
		err << "requested time " << time << " not found, nearest is "
				<< times[idx];
...
template<int dim>
void DiscretizedFunction<dim>::at(double time,
...
template<int dim>
double DiscretizedFunction<dim>::value(const Point<dim> &p,
		const unsigned int component) const {
	Assert(component == 0, ExcIndexRange(component, 0, 1));
	Assert(near_enough(Function<dim>::get_time(), cur_time_idx),
			ExcNotImplemented());
...
Tensor<1, dim, double> DiscretizedFunction<dim>::gradient(const Point<dim> &p,
	...
	return VectorTools::point_gradient(*dof_handlers[cur_time_idx],
			function_coefficients[cur_time_idx], p);
}
