...
WaveEquation<dim>::WaveEquation(DoFHandler<dim> *dof_hndl,
		const std::vector<double>& times, const Quadrature<dim> quad) :
		theta(0.5), dof_handler(dof_hndl), quad(quad), times(times), initial_values_u(
				zero), initial_values_v(zero), boundary_values_u(zero), boundary_values_v(
				zero), param_c(one), param_nu(zero), param_a(one), param_q(
				zero), right_hand_side(zero_rhs) {
}

template<int dim>
WaveEquation<dim>::WaveEquation(const WaveEquation<dim>& weq) :
		theta(weq.theta), dof_handler(weq.dof_handler), quad(weq.quad), times(
				weq.times), initial_values_u(weq.initial_values_u), initial_values_v(
				weq.initial_values_v), boundary_values_u(weq.boundary_values_u), boundary_values_v(
				weq.boundary_values_v), param_c(weq.param_c), param_nu(
				weq.param_nu), param_a(weq.param_a), param_q(weq.param_q), right_hand_side(
				weq.right_hand_side) {
}
...
void WaveEquation<dim>::setup_step(double time) {
	...
	task_group += Threads::new_task(&RightHandSide<dim>::create_right_hand_side,
			*right_hand_side, *dof_handler, quad, rhs);
	task_group.join_all();
}
...
void WaveEquation<dim>::fill_A() {
	...
	else // (param_a_disc && param_q_disc)
		MatrixCreator::create_laplace_mass_matrix(*dof_handler, quad, matrix_A,
						param_a_disc->get_function_coefficients()[param_a_disc->get_time_index()],
						param_q_disc->get_function_coefficients()[param_q_disc->get_time_index()]);
...
void WaveEquation<dim>::fill_B() {
	if (param_nu_disc && using_special_assembly())
		...
		dealii::MatrixCreator::create_mass_matrix(*dof_handler, quad, matrix_B,
				param_nu.get());
}
...
void WaveEquation<dim>::fill_C() {
	if (param_c_disc && using_special_assembly())
		...
		dealii::MatrixCreator::create_mass_matrix(*dof_handler, quad, matrix_C,
				param_c.get());
}
...
void WaveEquation<dim>::assemble_u(double time_step) {
	...
	VectorTools::interpolate_boundary_values(*dof_handler, 0,
			*boundary_values_u, boundary_values);
...
void WaveEquation<dim>::assemble_v(double time_step) {
	...
	std::map<types::global_dof_index, double> boundary_values;
	VectorTools::interpolate_boundary_values(*dof_handler, 0,
			*boundary_values_v, boundary_values);
...
DiscretizedFunction<dim> WaveEquation<dim>::run() {
	...
	for (size_t i = 1; i < times.size(); i++) {
		...
		deallog << "‖u‖=" << solution_u.l2_norm() << ", ‖v‖="
				<< solution_v.l2_norm() << std::endl;
		deallog.flags(f);
	}

	timer.stop();
	std::ios::fmtflags f(deallog.flags(std::ios_base::fixed));
	deallog << "solved pde in " << timer.wall_time() << "s (setup "
			<< setup_timer.wall_time() << "s)" << std::endl;
	deallog.flags(f);
...
template<int dim>
inline void WaveEquation<dim>::set_boundary_values_u(
		std::shared_ptr<Function<dim> > boundary_values_u) {
...
template<int dim>
inline void WaveEquation<dim>::set_boundary_values_v(
		std::shared_ptr<Function<dim>> boundary_values_v) {
...
template<int dim>
inline void WaveEquation<dim>::set_initial_values_u(
		std::shared_ptr<Function<dim>> initial_values_u) {
...
template<int dim>
inline void WaveEquation<dim>::set_initial_values_v(
		std::shared_ptr<Function<dim>> initial_values_v) {
...
template<int dim>
inline void WaveEquation<dim>::set_param_a(
		std::shared_ptr<Function<dim>> param_a) {
	this->param_a = param_a;
	this->param_a_disc = std::dynamic_pointer_cast<DiscretizedFunction<dim>,
			Function<dim>>(param_a);
}
...
template<int dim>
inline void WaveEquation<dim>::set_param_c(
		std::shared_ptr<Function<dim>> param_c) {
	this->param_c = param_c;
	this->param_c_disc = std::dynamic_pointer_cast<DiscretizedFunction<dim>,
			Function<dim>>(param_c);
}
...
template<int dim>
inline void WaveEquation<dim>::set_param_nu(
		std::shared_ptr<Function<dim>> param_nu) {
	this->param_nu = param_nu;
	this->param_nu_disc = std::dynamic_pointer_cast<DiscretizedFunction<dim>,
			Function<dim>>(param_nu);
}
...
template<int dim>
inline void WaveEquation<dim>::set_param_q(
		std::shared_ptr<Function<dim>> param_q) {
	this->param_q = param_q;
	this->param_q_disc = std::dynamic_pointer_cast<DiscretizedFunction<dim>,
			Function<dim>>(param_q);
}
...
template<int dim>
inline void WaveEquation<dim>::set_right_hand_side(
...
template<int dim> void WaveEquation<dim>::set_times(
...
template<int dim> void WaveEquation<dim>::set_special_assembly_tactic(
		int special_assembly_tactic) {
