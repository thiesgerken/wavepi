...
template<int dim>
L2ProductRightHandSide<dim>::L2ProductRightHandSide(DiscretizedFunction<dim>* f1, DiscretizedFunction<dim>* f2)
      : func1(f1), func2(f2) {
}
...
void L2ProductRightHandSide<dim>::create_right_hand_side(const DoFHandler<dim> &dof, const Quadrature<dim> &quad,
   ...
   WorkStream::run(dof.begin_active(), dof.end(),
         std::bind(&local_assemble<dim>, std::ref(coeffs1), std::ref(coeffs2), std::placeholders::_1,
