...
DiscretizedFunction<dim>::DiscretizedFunction(std::shared_ptr<SpaceTimeMesh<dim>> mesh,
      std::shared_ptr<Norm<DiscretizedFunction<dim>>> norm, bool store_derivative)
...
DiscretizedFunction<dim>::DiscretizedFunction(DiscretizedFunction<dim> && o)
      : LightFunction<dim>(), mesh(std::move(o.mesh)), norm_(o.norm_), store_derivative(o.store_derivative),
            cur_time_idx(o.cur_time_idx), function_coefficients(std::move(o.function_coefficients)),
...
DiscretizedFunction<dim>::DiscretizedFunction(const DiscretizedFunction<dim>& o)
      : LightFunction<dim>(), mesh(o.mesh), norm_(o.norm_), store_derivative(o.store_derivative),
            cur_time_idx(o.cur_time_idx), function_coefficients(o.function_coefficients),
   ...
   for (size_t i = 0; i < mesh->length(); i++)
      for (size_t j = 0; j < function_coefficients[i].size(); j++)
         if (function_coefficients[i][j] != 0.0)
            function_coefficients[i][j] = std::pow(std::abs(function_coefficients[i][j]), p - 2)
