...
template <int dim>
GridPointMeasure<dim>::GridPointMeasure() : PointMeasure<dim>() {}
...
void GridPointMeasure<dim>::get_parameters(ParameterHandler &prm) {
  prm.enter_subsection("GridPointMeasure");
  {
    ...
    if (dim > 1) spatial_points.emplace_back(make_points(prm.get("points y")));

    if (dim > 2) spatial_points.emplace_back(make_points(prm.get("points z")));
