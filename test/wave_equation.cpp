...
void run_discretized_test(int fe_order, int quad_order, int refines) {
	...
	std::shared_ptr<SpaceTimeMesh<dim>> mesh = std::make_shared<
			ConstantMesh<dim>>(times, dof_handler, quad);
	...
	TestC<dim> c;
	auto c_disc = std::make_shared<DiscretizedFunction<dim>>(mesh, dof_handler,
			c);
	wave_eq.set_param_c(c_disc);

	TestA<dim> a;
	auto a_disc = std::make_shared<DiscretizedFunction<dim>>(mesh, dof_handler,
			a);
	wave_eq.set_param_a(a_disc);

	TestQ<dim> q;
	auto q_disc = std::make_shared<DiscretizedFunction<dim>>(mesh, dof_handler,
			q);
	wave_eq.set_param_q(q_disc);

	TestNu<dim> nu;
	auto nu_disc = std::make_shared<DiscretizedFunction<dim>>(mesh, dof_handler,
			nu);
	wave_eq.set_param_nu(nu_disc);

	TestF<dim> f;
	auto f_disc = std::make_shared<DiscretizedFunction<dim>>(mesh, dof_handler,
			f);
...
void run_l2adjoint_test(int fe_order, int quad_order, int refines,
	...
	double t_start = 0.0, t_end = 2.0, dt = 1.0 / n_steps;
	std::vector<double> times;
	...
	TestF<dim> f_cont;
	auto f = std::make_shared<DiscretizedFunction<dim>>(mesh, dof_handler,
			f_cont);
	...
	TestG<dim> g_cont;
	auto g = std::make_shared<DiscretizedFunction<dim>>(mesh, dof_handler,
			g_cont);
...
public:
	double value(const Point<dim> &p, const unsigned int component = 0) const {
		...
		res *= constants[0] * std::sin(std::sqrt(k.square()) * this->get_time())
				+ constants[1]
						* std::cos(std::sqrt(k.square()) * this->get_time());
		return res;
	}
...
template<int dim>
void run_reference_test(int fe_order, int quad_order, int refines,
		Point<dim, int> k, Point<2> constants, double t_end, int steps,
		bool expect = true) {
	std::ofstream logout("wavepi_test.log", std::ios_base::app);
	deallog.attach(logout);
	deallog.depth_console(0);
	...
	deallog << std::scientific << "forward : rerr(u) = " << err_u
			<< ", rerr(v) = " << err_v << std::endl;
	...
	deallog << std::scientific << "backward: rerr(u) = " << err_u
			<< ", rerr(v) = " << err_v << std::endl << std::endl;
}
...
TEST(WaveEquationTest, ReferenceTest1DFE1) {
	for (int steps = 128; steps <= 1024; steps *= 2)
		run_reference_test<1>(1, 3, 10, Point<1, int>(2), Point<2>(1.0, 1.5),
				2 * numbers::PI, steps, steps >= 64);

	for (int refine = 9; refine >= 1; refine--)
		run_reference_test<1>(1, 3, refine, Point<1, int>(2),
				Point<2>(1.0, 1.5), 2 * numbers::PI, 1024, false);
}

TEST(WaveEquationTest, ReferenceTest1DFE2) {
	for (int steps = 16; steps <= 128; steps *= 2)
		run_reference_test<1>(2, 4, 7, Point<1, int>(2), Point<2>(1.0, 1.5),
				2 * numbers::PI, steps, steps >= 64);

	for (int refine = 6; refine >= 1; refine--)
		run_reference_test<1>(2, 4, refine, Point<1, int>(2),
				Point<2>(1.0, 1.5), 2 * numbers::PI, 128, false);
}

TEST(WaveEquationTest, ReferenceTest2DFE1) {
	for (int steps = 16; steps <= 256; steps *= 2)
		run_reference_test<2>(1, 3, 6, Point<2, int>(1, 2), Point<2>(1.0, 1.5),
				2 * numbers::PI, steps, steps >= 64);

	for (int refine = 5; refine >= 1; refine--)
		run_reference_test<2>(1, 3, refine, Point<2, int>(1, 2),
				Point<2>(1.0, 1.5), 2 * numbers::PI, 256, false);
}

TEST(WaveEquationTest, ReferenceTest3DFE1) {
	for (int steps = 8; steps <= 32; steps *= 2)
		run_reference_test<3>(1, 3, 3, Point<3, int>(1, 2, 3),
				Point<2>(0.7, 1.2), 2 * numbers::PI, steps, steps >= 32);

	for (int refine = 2; refine >= 0; refine--)
		run_reference_test<3>(1, 3, refine, Point<3, int>(1, 2, 3),
				Point<2>(0.7, 1.2), 2 * numbers::PI, 32, false);
}
