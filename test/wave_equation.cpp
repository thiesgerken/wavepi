...
void run_discretized_test(int fe_order, int quad_order, int refines) {
   ...
   deallog << "rel. error between continuous and full discrete: " << std::scientific << err_cont_vs_disc
         << std::endl;
...
void run_l2adjoint_test(int fe_order, int quad_order, int refines) {
   ...
   deallog << std::scientific << "(Lf, g) = " << dot_solf_g << ", (f, L*g) = " << dot_f_adjg
         << ", rel. error = " << fg_err << std::endl << std::endl;
}
...
TEST(WaveEquationTest, ReferenceTest1DFE1) {
   for (int steps = 128; steps <= 2048; steps *= 2)
      run_reference_test<1>(1, 3, 10, Point<1, int>(2), Point<2>(1.0, 1.5), 2 * numbers::PI, steps,
            steps >= 64);

   for (int refine = 9; refine >= 1; refine--)
      run_reference_test<1>(1, 3, refine, Point<1, int>(2), Point<2>(1.0, 1.5), 2 * numbers::PI, 2048, false);
}
