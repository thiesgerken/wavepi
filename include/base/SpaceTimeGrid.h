...
class SpaceTimeGrid {
 public:
   ...
   */
  const std::vector<std::vector<double>>& get_grid_extents() const { return grid_extents; }

  const std::vector<double>& get_times() const { return times; }

  const std::vector<Point<dim + 1>>& get_space_time_points() const { return space_time_points; }

  const std::vector<std::vector<Point<dim>>>& get_points() const { return points; }
