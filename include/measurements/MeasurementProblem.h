...
class MeasurementProblem: public NonlinearProblem<Param, Measurement> {
   public:
      ...
      virtual std::unique_ptr<LinearProblem<Param, Measurement>> derivative(const Param& p) {
         return std::make_unique<MeasurementProblem<Param, Sol, Measurement>::Linearization>(
               base->derivative(p), measure);
      }

      virtual Measurement forward(const Param& x) {
         return measure->evaluate(base->forward(x));
      }
   ...
   private:
      ...
      class Linearization: public LinearProblem<Param, Measurement> {
         public:
            ...
            Linearization(std::shared_ptr<LinearProblem<Param, Sol>> base,
                  std::shared_ptr<Measure<Sol, Measurement>> measure)
                  : base(base), measure(measure) {
               Assert(base, ExcInternalError());
            }

            virtual Measurement forward(const Param& h) {
               return measure->evaluate(base->forward(h));
            }

            virtual Param adjoint(const Measurement& g) {
               return base->adjoint(measure->adjoint(g));
            }
