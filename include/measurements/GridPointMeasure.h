...
class GridPointMeasure : public PointMeasure<dim> {
 public:
   ...
   * @param spatial_points a vector with `dim` entries, each containing the coordinates for measurement points in that
   * dimension.
   ...
   */
  GridPointMeasure(const std::vector<double> &times, const std::vector<std::vector<double>> &spatial_points,
