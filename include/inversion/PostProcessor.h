...
class PostProcessor {
 public:
  ...
  virtual void post_process(int iteration_number, Param *current_estimate, double norm_current_estimate) = 0;
};
...
class BoundEnforcingPostProcessor : public PostProcessor<DiscretizedFunction<dim>> {
 public:
  ...
  BoundEnforcingPostProcessor(ParameterHandler &prm) { get_parameters(prm); }
  ...
  virtual void post_process(int iteration_number __attribute__((unused)), DiscretizedFunction<dim> *current_estimate,
    ...
    if (lower_bound <= -1e100 && upper_bound >= 1e100) return;
    ...
    for (size_t i = 0; i < current_estimate->length(); i++)
      for (size_t j = 0; j < (*current_estimate)[i].size(); j++) {
        if (lower_bound > (*current_estimate)[i][j]) {
          if (est_min > (*current_estimate)[i][j]) est_min = (*current_estimate)[i][j];

          (*current_estimate)[i][j] = lower_bound;
        }

        if (upper_bound < (*current_estimate)[i][j]) {
          if (est_max < (*current_estimate)[i][j]) est_max = (*current_estimate)[i][j];
    ...
    if (est_min < lower_bound)
      deallog << "Adjusted lower bound of estimate from " << est_min << " to " << lower_bound << std::endl;

    if (est_max > upper_bound)
      deallog << "Adjusted upper bound of estimate from " << est_max << " to " << upper_bound << std::endl;
  }
