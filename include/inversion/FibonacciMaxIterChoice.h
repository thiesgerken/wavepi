...
/**
 * Implements the max iter choice i^n_max = i^{n-1} + i^n, starting with given i^0_max (defaults to 1) and i^1_max =
 * 2*i^0_max. Idea is due to Winkler („A Model-Aware Inexact Newton Scheme for Electrical Impedance Tomography“, 2016)
 * I call it FibonacciMaxIterChoice because in the worst case the iteration counts are identical to the Fibonacci
 * sequence.
 */
