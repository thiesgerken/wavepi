...
class AdaptiveMesh: public SpaceTimeMesh<dim> {
   public:
      ...
      std::shared_ptr<DoFHandler<dim> > get_dof_handler(int time_index);
      ...
      std::shared_ptr<DoFHandler<dim> > transfer_to(int source_time_index, int target_time_index,
            std::vector<shared_ptr<Vector<double>>> vectors);
   ...
   private:
      ...
      std::shared_ptr<DoFHandler<dim>> working_triangulation;
