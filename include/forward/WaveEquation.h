...
class WaveEquation {
public:
	WaveEquation(DoFHandler<dim> *dof_handler, const std::vector<double> &times,
	...
	std::shared_ptr<Function<dim>> zero = std::make_shared<ZeroFunction<dim>>(
			1);
	std::shared_ptr<Function<dim>> one =
			std::make_shared<ConstantFunction<dim>>(1.0, 1);

	std::shared_ptr<RightHandSide<dim>> zero_rhs = std::make_shared<
			L2RightHandSide<dim>>(zero);
	...
	inline bool using_special_assembly() {
		return special_assembly_tactic == 0 ?
				is_special_assembly_recommended() :
...
private:
	...
	std::shared_ptr<DiscretizedFunction<dim>> param_c_disc = nullptr,
			param_nu_disc = nullptr;
	std::shared_ptr<DiscretizedFunction<dim>> param_a_disc = nullptr,
			param_q_disc = nullptr;
