...
class WaveEquationBase {
 public:
  ...
  inline bool is_special_assembly_recommended() const { return mesh->get_quadrature().size() < (1 << dim) || dim < 3; }

  inline bool using_special_assembly() {
    return special_assembly_tactic == 0 ? is_special_assembly_recommended() : (special_assembly_tactic > 0);
  }
  ...
  inline std::shared_ptr<Function<dim>> get_param_c() const { return param_c; }
  ...
  inline std::shared_ptr<Function<dim>> get_param_nu() const { return param_nu; }
  ...
  inline std::shared_ptr<Function<dim>> get_param_q() const { return param_q; }
  ...
  inline std::shared_ptr<RightHandSide<dim>> get_right_hand_side() const { return right_hand_side; }
