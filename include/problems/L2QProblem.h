...
#ifndef INCLUDE_PROBLEMS_L2QPROBLEM_H_
#define INCLUDE_PROBLEMS_L2QPROBLEM_H_
...
class L2QProblem: public L2WaveProblem<dim, Measurement> {
   public:
      ...
      L2QProblem(WaveEquation<dim>& weq, std::vector<std::shared_ptr<Function<dim>>> right_hand_sides,
   ...
   protected:
      virtual std::unique_ptr<LinearProblem<DiscretizedFunction<dim>, DiscretizedFunction<dim>>> derivative(
            size_t i) {
         return std::make_unique<L2QProblem<dim, Measurement>::Linearization>(this->wave_equation,
      ...
      class Linearization: public LinearProblem<DiscretizedFunction<dim>, DiscretizedFunction<dim>> {
         public:
            ...
            Linearization(const WaveEquation<dim> &weq,
                  ...
                  : weq(weq), weq_adj(weq), normX(q->get_norm()), normY(u->get_norm()), adjoint_solver(
                        adjoint_solver) {
               this->q = q;
               this->u = u;
