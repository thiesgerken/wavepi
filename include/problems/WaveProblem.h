...
public:
   ...
   WaveProblem(const WaveEquation<dim> &weq, std::vector<std::shared_ptr<Function<dim>>> right_hand_sides,
         ...
         : stats(std::make_shared<NonlinearProblemStats>()), wave_equation(weq), background_param(background_param),
               adjoint_solver(WaveEquationBase<dim>::WaveEquationAdjoint), right_hand_sides(right_hand_sides),
               measures(measures), transform(transform) {
   ...
   std::shared_ptr<Norm<DiscretizedFunction<dim>>> get_norm_domain() const {
      return norm_domain;
   }
    ...
    */
   void set_norm_domain(std::shared_ptr<Norm<DiscretizedFunction<dim>>> norm_domain) {
    ...
    *
    */
   std::shared_ptr<Norm<DiscretizedFunction<dim>>> get_norm_codomain() const {
   ...
   const typename WaveEquationBase<dim>::L2AdjointSolver& get_adjoint_solver() const {
...
private:
   ...
   public:
      ...
      Linearization(std::vector<std::shared_ptr<LinearizedSubProblem<dim>>> sub_problems,
            ...
            std::shared_ptr<Norm<DiscretizedFunction<dim>>> norm_domain,
            std::shared_ptr<Norm<DiscretizedFunction<dim>>> norm_codomain)
            : stats(std::make_shared<LinearProblemStats>()), parent_stats(parent_stats), sub_problems(sub_problems),
                  measures(measures), current_param_transformed(current_param_transformed), transform(transform),
                  norm_domain(norm_domain), norm_codomain(norm_codomain) {
      }
